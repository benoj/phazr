...
class Orchestrator:
    ...
    def _register_default_handlers(self):
        ...
        self.handler_registry.register(OperationType.KUBECTL_RESTART, KubectlRestartHandler())
    ...
    async def run_full_setup(self, version: Optional[str] = None) -> List[PhaseResult]:
        ...
        for phase in self.config.phases:
            ...
            # Check dependencies
            if phase.depends_on:
                missing_deps = [dep for dep in phase.depends_on if dep not in completed_phases]
                if missing_deps:
                    self.display.warning(f"Skipping phase {phase.name} - missing dependencies: {missing_deps}")
                    continue
            ...
            if not phase_result.is_successful and not phase.continue_on_error and not self.config.execution.continue_on_error:
    ...
    async def run_phase(
        ...
        total_ops = sum(
            len(version_config.groups.get(group, []))
        ...
        for group_name in phase_groups:
            ...
            if not operations:
                self.display.warning(f"Group '{group_name}' not found in version config")
                continue
            
            # Determine if we should run in parallel
            should_parallel = phase.parallel_groups or (self.config.execution.parallel and self._is_group_parallelizable(operations))
            
            if should_parallel:
                results = await self._execute_parallel(operations, operation_index, total_ops)
            else:
                results = await self._execute_sequential(operations, operation_index, total_ops)
            ...
            if failed and not phase.continue_on_error and not self.config.execution.continue_on_error:
        ...
        phase_result = PhaseResult(
            ...
            failed_operations=sum(1 for r in all_results if not r.success and r.operation.type != OperationType.SKIP),
            skipped_operations=sum(1 for r in all_results if r.operation.type == OperationType.SKIP),
    ...
    async def _execute_sequential(self, operations: List[Operation], start_index: int = 0, total: int = None) -> List[ExecutionResult]:
    ...
    async def _execute_parallel(self, operations: List[Operation], start_index: int = 0, total: int = None) -> List[ExecutionResult]:
        ...
        for operation in operations:
            if self.config.execution.dry_run:
                task = asyncio.create_task(
                    asyncio.coroutine(lambda op=operation: self._create_dry_run_result(op))()
                )
            else:
        ...
        for chunk in self._chunk_list(operation_tasks, self.config.execution.max_parallel):
            chunk_ops = [op for op, _ in chunk]
        ...
        while retries <= operation.retry_count:
            try:
                ...
                if retries <= operation.retry_count:
                    self.logger.warning(f"Retry {retries}/{operation.retry_count} for {operation.description}")
