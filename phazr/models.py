...
class Phase(BaseModel):
    ...
    groups: List[str] = Field(default_factory=list, description="Operation groups in this phase")
    depends_on: List[str] = Field(default_factory=list, description="Phases that must complete before this")
    continue_on_error: bool = Field(False, description="Continue to next phase even if this fails")
...
class Operation(BaseModel):
    ...
    service: Optional[str] = Field(None, description="Service/deployment name for kubectl operations")
    namespace: Optional[str] = Field(None, description="Kubernetes namespace override")
    container: Optional[str] = Field(None, description="Container name for kubectl exec")
    ...
    # Test and validation
    test_command: Optional[str] = Field(None, description="Command to test if operation succeeded")
...
class ExecutionConfig(BaseModel):
    ...
    parallel: bool = Field(False, description="Execute operations in parallel where safe")
