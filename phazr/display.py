...
from .models import Phase, PhaseResult, ExecutionResult, Operation
...
╚═══════════════════════════════════════════════════════════════════════╝[/bold cyan]
    ...
    def print_config_info(self, config: Dict[str, Any]):
        ...
        table.add_row("Environment", config.get("environment", {}).get("name", "unknown"))
        table.add_row("Namespace", config.get("environment", {}).get("namespace", "default"))
        
        if config.get("environment", {}).get("context"):
            table.add_row("Context", config["environment"]["context"])
        
        table.add_row("Dry Run", "Yes" if config.get("execution", {}).get("dry_run") else "No")
        table.add_row("Parallel", "Yes" if config.get("execution", {}).get("parallel") else "No")
    ...
    def show_operation_progress(self, operation: Operation, index: int, total: int) -> Any:
    ...
    def show_operation_result(self, result: ExecutionResult, index: int, total: int):
        ...
        self.console.print(f"║      [{status_color}]{padded_result}[/{status_color}]  ║")
    ...
    def show_phase_summary(self, phase_result: PhaseResult):
        ...
        duration = time.time() - self._start_time if self._start_time else phase_result.duration
    ...
    def show_final_summary(self, results: List[PhaseResult]):
        ...
        self.console.print()
        self.console.print("╔══════════════════════════════════════════════════════════════════════╗")
        self.console.print("║                        📊 EXECUTION SUMMARY                          ║")
        self.console.print("╠══════════════════════════════════════════════════════════════════════╣")
        ...
        self.console.print(f"║[bold {total_color}]{total_line:<70}[/bold {total_color}]║")
        self.console.print("╚══════════════════════════════════════════════════════════════════════╝")
