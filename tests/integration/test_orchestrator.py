...
from tests.utils.test_helpers import create_successful_result, create_failed_result
...
class TestOrchestrator:
    ...
    def mock_handler_registry(self):
        ...
        registry.register(OperationType.KUBECTL_EXEC, MockOperationHandler(success=True))
        registry.register(OperationType.HTTP_REQUEST, MockOperationHandler(success=True))
    ...
    @pytest.fixture
    def orchestrator(self, sample_orchestrator_config_with_phases, mock_handler_registry, mock_display):
    ...
    @pytest.mark.asyncio
    async def test_orchestrator_initialization(self, sample_orchestrator_config_with_phases):
    ...
    async def test_error_handling_continue_on_error(self, mock_handler_registry):
        ...
        mock_handler_registry.register(
            ...
            MockOperationHandler(success=False)
        )
    ...
    @pytest.mark.asyncio
    async def test_dry_run_mode(self, sample_orchestrator_config_with_phases, mock_handler_registry):
    ...
    @pytest.mark.asyncio
    async def test_disabled_phase_skipping(self, sample_orchestrator_config_with_phases, mock_handler_registry):
    ...
    async def test_complex_dependency_graph(self, mock_handler_registry):
        ...
        operations = {
            "setup_ops": [Operation(command="setup", description="Setup", type=OperationType.SCRIPT_EXEC)],
            "build_ops": [Operation(command="build", description="Build", type=OperationType.SCRIPT_EXEC)],
            "lint_ops": [Operation(command="lint", description="Lint", type=OperationType.SCRIPT_EXEC)],
            "deploy_ops": [Operation(command="deploy", description="Deploy", type=OperationType.KUBECTL_EXEC, service="app")]
        }
        ...
        phases = [
            ...
            Phase(name="build", groups=["build_ops"], depends_on=["setup"], enabled=True),
            Phase(name="lint", groups=["lint_ops"], depends_on=["setup"], enabled=True),
            Phase(name="deploy", groups=["deploy_ops"], depends_on=["build", "lint"], enabled=True)
        ]
    ...
    async def test_version_selection(self, mock_handler_registry):
        ...
        v1_operations = {
            "build": [Operation(command="build v1", description="Build v1", type=OperationType.SCRIPT_EXEC)]
        }
        v2_operations = {
            "build": [Operation(command="build v2", description="Build v2", type=OperationType.SCRIPT_EXEC)]
        }
...
class TestOrchestratorErrorScenarios:
    ...
    async def test_circular_dependency_detection(self):
        ...
        phases = [
            Phase(name="phase_a", groups=["group_a"], depends_on=["phase_b"], enabled=True),
            Phase(name="phase_b", groups=["group_b"], depends_on=["phase_c"], enabled=True),
            Phase(name="phase_c", groups=["group_c"], depends_on=["phase_a"], enabled=True)  # Circular!
        ]
        
        operations = {
            "group_a": [Operation(command="a", description="A", type=OperationType.SCRIPT_EXEC)],
            "group_b": [Operation(command="b", description="B", type=OperationType.SCRIPT_EXEC)],
            "group_c": [Operation(command="c", description="C", type=OperationType.SCRIPT_EXEC)]
        }
    ...
    async def test_empty_phase_handling(self):
        ...
        phases = [
            Phase(name="empty_phase", groups=["nonexistent_group"], enabled=True)
        ]
